...
def test_chat_session_compare_uses_last_two_runs_without_manual_run_id(tmp_path: Path) -> None:
    ...
    first = client.post(
        "/chat/message",
        json={"message": "build a trend strategy and run backtest", "include_debug": True},
    )
    assert first.status_code == 200
    session_id = first.json()["session_id"]
    first_run_id = str(first.json()["debug"]["run_id"])

    second = client.post(
        "/chat/message",
        json={"session_id": session_id, "message": "double commission and slippage then run again", "include_debug": True},
    )
    assert second.status_code == 200
    second_payload = second.json()
    assert second_payload["mode"] == "modify_last_run"
    assert second_payload["debug"]["intent"] == "modify_last_run"
    second_run_id = str(second_payload["debug"]["run_id"])
    assert second_run_id != first_run_id

    third = client.post(
        "/chat/message",
        json={"session_id": session_id, "message": "why worse than last run?", "include_debug": True},
    )
    assert third.status_code == 200
    payload = third.json()
    assert payload["mode"] == "session_compare"
    assert payload["debug"]["intent"] == "session_compare"
    assert "run_compare" in payload["debug"]

    card_types = {row["type"] for row in payload["cards"]}
    assert "summary" in card_types
    assert "metrics" in card_types
    assert "diff" in card_types

    compare = payload["debug"]["run_compare"]
